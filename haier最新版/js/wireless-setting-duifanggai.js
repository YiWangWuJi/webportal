...
var wifiIndex = {
		 ...
		 */
		init : function() {
			token = "?token=e97fe4bcbae14ad48ec0624ae1360f8c"
		...
		radioInfoCallBack : function(data,band) {
			...
			if ((null != _2GSimInfo && null != _5GSimInfo && _2GSimInfo && _5GSimInfo) 
			   && (null != _2GRadioInfo && null != _5GRadioInfo && _2GRadioInfo && _5GRadioInfo)) {
				if ((_2GSimInfo.hidden == _5GSimInfo.hidden) && (_2GSimInfo.key == _5GSimInfo.key) && (_2GSimInfo.encryption == _5GSimInfo.encryption)
		...
		savewWifitInfo : function() {
			...
			callRpc(rpcUrl + this.paramOperate.businessType + token, this.paramOperate.method.enableWiFiInfo, 'double', function(data){console.log('5G-radio--->' + data);});
		},
		/**
		 * 根据band获取wifi基本配置
		...
		getWifiSimValWithBand : function(band) {
			...
			if ($('#same_2G_icon').hasClass('selected')) {
				var hidden   = $('icon[name="2G_hidden"]').hasClass('selected') ? true : false;                           //是否隐藏
				var key      = $('input[name="2G_password"]').val();                                                         //密码
				var authMode = $('#2G_authMode').find('h3').attr('value');
				var encType  = $('#2G_enctype').find('h3').attr('value');
			} else {
				var hidden   = $('icon[name='  + band + '_hidden]').hasClass('selected') ? true : false;                           //是否隐藏
				var key      = $('input[name=' + band + '_password]').val();                                                         //密码
				var authMode = $('#' + band + '_authMode').find('h3').attr('value');
				var encType  = $('#' + band + '_enctype').find('h3').attr('value');
			}
			var enable = $('#' + band + '_enable').hasClass('selected') ? false : true;                                       //是否启用
			var ssid   = $('input[name=' + band + '_ssid]').val();                                                            //ssid
			var encryption = authMode + '+' + encType;  
		...
		getWifiRadioValWithBand : function(band) {
			...
			params = [band, channel, htmode, txpower, beacon, mode];
			return params;
		},
}
//2G基本信息临时cache
